...
export const OrderCreation: React.FC<OrderCreationProps> = ({
  ...
  const handleSubmit = () => {
    if (cart.length === 0) {
      toast({ title: "Empty Cart", description: "Please add items to create an order", variant: "destructive" });
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[100vw] sm:max-w-[95vw] md:max-w-4xl lg:max-w-6xl h-[100vh] sm:h-auto sm:max-h-[95vh] w-full p-0 flex flex-col rounded-none sm:rounded-lg">
        <DialogHeader className="p-4 sm:p-6 pb-3 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 flex-shrink-0">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-2 lg:hidden">
              <Button variant={currentView === 'menu' ? 'default' : 'outline'} size="sm" onClick={() => setCurrentView('menu')} className="flex-1 h-10"><Menu className="h-4 w-4 mr-2" />{language === 'hi' ? 'मेन्यू' : 'Menu'}</Button>
              <Button variant={currentView === 'cart' ? 'default' : 'outline'} size="sm" onClick={() => setCurrentView('cart')} className="flex-1 h-10 relative"><ShoppingCart className="h-4 w-4 mr-2" />{language === 'hi' ? 'कार्ट' : 'Cart'}{cart.length > 0 && (<Badge variant="secondary" className="ml-2 text-xs min-w-[20px] h-5">{cart.length}</Badge>)}</Button>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0">
          {/* Menu Section */}
          <div className={`flex-1 p-4 sm:p-6 flex flex-col gap-4 ${currentView === 'cart' ? 'hidden lg:flex' : 'flex'}`}>
              ...
              {orderType === 'dine-in' && (
                <div>
                  ...
                  <Select value={selectedTableId} onValueChange={setSelectedTableId}>
                    ...
                    <SelectContent>
                      {tables.filter(t => t.status === 'available').map((table) => (<SelectItem key={table.id} value={table.id}>{language === 'hi' ? 'टेबल' : 'Table'} {table.number}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
              )}
            ...
            {/* ✅ FIX: Replaced complex manual scrolling structure with ScrollArea */}
            <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="flex flex-col flex-1 min-h-0">
              <ScrollArea className="w-full whitespace-nowrap shrink-0 pb-2">
                <TabsList className="h-10">
                  <TabsTrigger value="">{language === 'hi' ? 'सभी' : 'All'}</TabsTrigger>
                  {categories.map((category) => (<TabsTrigger key={category.id} value={category.name.toLowerCase().replace(/\s+/g, '-')}>{category.name}</TabsTrigger>))}
                </TabsList>
              </ScrollArea>
              <ScrollArea className="flex-1 mt-4">
                <TabsContent value={selectedCategory} className="mt-0">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 pr-2">
                        ...
                        <CardContent className="p-3 sm:p-4">
                          <div className="space-y-3">
                            <div className="flex justify-between items-start gap-2">
                              <h4 className="font-medium text-sm sm:text-base flex-1 line-clamp-2 leading-snug">{getLocalizedText(item.name, language)}</h4>
                              <Badge variant="secondary" className="text-xs font-semibold shrink-0 px-2 py-1">₹{item.price}</Badge>
                            </div>
                            {item.description && (<p className="text-xs sm:text-sm text-muted-foreground line-clamp-2 leading-relaxed">{getLocalizedText(item.description, language)}</p>)}
                            <Button size="sm" className="w-full text-xs sm:text-sm h-8 sm:h-9 font-medium" tabIndex={-1}><Plus className="h-3 w-3 sm:h-4 sm:w-4 mr-1.5" />{language === 'hi' ? 'जोड़ें' : 'Add'}</Button>
                          </div>
                        </CardContent>
                      </Card>
                    ))}
          ...
          {/* Cart Section */}
          <div className={`w-full lg:w-80 xl:w-96 border-t lg:border-l lg:border-t-0 bg-muted/30 flex flex-col ${currentView === 'menu' ? 'hidden lg:flex' : 'flex'}`}>
            <div className="p-4 sm:p-6 flex items-center gap-2 shrink-0 border-b">
                    ...
                    {cart.map((item) => (
                      <Card key={item.id} className="transition-all duration-200 hover:shadow-sm">
                        <CardContent className="p-3 sm:p-4">
                          <div className="space-y-3">
                            <div className="flex justify-between items-start gap-2">
                              <h5 className="font-medium text-sm sm:text-base line-clamp-2 flex-1">{getLocalizedText(item.menuItem.name, language)}</h5>
                              <Button variant="ghost" size="sm" onClick={() => removeFromCart(item.id)} className="h-7 w-7 shrink-0 hover:bg-destructive/10 hover:text-destructive"><X className="h-4 w-4" /></Button>
                            </div>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <Button variant="outline" size="sm" onClick={() => updateQuantity(item.id, item.quantity - 1)} className="h-8 w-8 hover:bg-muted"><Minus className="h-3 w-3" /></Button>
                                <span className="w-8 text-center text-sm font-medium">{item.quantity}</span>
                                <Button variant="outline" size="sm" onClick={() => updateQuantity(item.id, item.quantity + 1)} className="h-8 w-8 hover:bg-muted"><Plus className="h-3 w-3" /></Button>
                              </div>
            ...
            {cart.length > 0 && (
              <div className="p-4 sm:p-6 shrink-0 space-y-3 pt-3 border-t">
